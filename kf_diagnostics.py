...
def kf_track_rmse_m(final_code, td_noise_cm, doa_noise_deg, target_dist_cm, tdoa_bias_cm):
    ...
    ls_init = final_code.compute_initial_kf_state(feat_cm, final_code.SENSORS_CM)
    kf = final_code.KalmanFilter(ls_init)
    kf_positions = [ls_init.copy()]

    for t in range(1, 200):
        ls_pos = final_code.solve_ls_localization(feat_cm[t, 2:9], final_code.SENSORS_CM)
        kf_positions.append(kf.predict_and_update(ls_pos))

    rmse_cm = final_code.calculate_rmse(gt_cm, np.array(kf_positions))
...
def main():
    parser = argparse.ArgumentParser(description="KF-only RMSE distribution diagnostics.")
