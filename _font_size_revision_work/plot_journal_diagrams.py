...
SENSOR_FONT_SIZE = 9.3
TRANSFORMER_FONT_SIZE = 14.0
    ...
    plt.rcParams.update(
        {
            ...
            "font.size": SENSOR_FONT_SIZE,
            "axes.labelsize": SENSOR_FONT_SIZE,
            "xtick.labelsize": SENSOR_FONT_SIZE,
            "ytick.labelsize": SENSOR_FONT_SIZE,
            "axes.linewidth": 0.7,
            "pdf.fonttype": 42,
            "ps.fonttype": 42,
...
def plot_sensor_array(output_dir: Path, dpi: int) -> None:
    ...
    ax.text(
        -0.1, 0.0, 0.65, r"$z=0$", fontsize=SENSOR_FONT_SIZE, color="#555555"
    )
    ax.text(
        -0.2,
        0.0,
...
def add_box(
    ...
    *,
    ...
    linewidth: float = 0.8,
    title_size: float = TRANSFORMER_FONT_SIZE,
    subtitle_size: float = TRANSFORMER_FONT_SIZE,
) -> None:
...
def plot_transformer(output_dir: Path, dpi: int) -> None:
    ...
    ax.text(
        ...
        ha="center",
        va="center",
        fontsize=TRANSFORMER_FONT_SIZE,
        color="#355A85",
        fontweight="semibold",
    )
    ax.text(
        4.60,
        (encoder_bottom + encoder_top) / 2.0,
        r"$\times\,9$; encoder dropout $=0.0534$",
        ha="center",
        va="center",
        fontsize=TRANSFORMER_FONT_SIZE,
        color="#355A85",
    ...
    add_box(
        ...
        r"$\mathbf{X}\in\mathbb{R}^{T\times F_{\mathrm{in}}}$; $T=20$" "\n"
        r"$F_{\mathrm{in}}=25$ in simulation",
        facecolor="#F7F7F7",
    )
    add_box(
        ax,
        ...
        r"scaled by $\sqrt{d_{\mathrm{model}}}$",
        facecolor="#EAF2F8",
    )
    add_box(
        ax,
    ...
    for lower_key, upper_key in zip(ordered[:-1], ordered[1:]):
        lower_height = height * height_factors.get(lower_key, 1.0)
        upper_height = height * height_factors.get(upper_key, 1.0)
        arrow(
            ax,
            center,
    ...
    for source_key, target_key in [("attention", "norm1"), ("ffn", "norm2")]:
        prev_key = ordered[ordered.index(source_key) - 1]
        prev_height = height * height_factors.get(prev_key, 1.0)
        source_height = height * height_factors.get(source_key, 1.0)
        arrow_y0 = positions[prev_key] + prev_height / 2.0 + 0.04
        arrow_y1 = positions[source_key] - source_height / 2.0 - 0.04
        source_y = (arrow_y0 + arrow_y1) / 2.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--diagram",
        choices=("all", "sensor", "transformer"),
        default="all",
    )
...
def main() -> None:
    args = parse_args()
    configure_style()
    if args.diagram in ("all", "sensor"):
        plot_sensor_array(args.output, args.dpi)
    if args.diagram in ("all", "transformer"):
