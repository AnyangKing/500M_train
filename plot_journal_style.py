...
def plot_rmse_figures(
    ...
    mask = (dist_m >= 100.0) & (dist_m <= 300.0)
    fig, ax = plt.subplots(figsize=(5.4, 3.45), constrained_layout=True)
    for key in MODEL_ORDER:
        plot_model_line(
            ax,
            dist_m[mask],
            results[f"r_dist_{key}"][mask],
            key,
            markevery=2,
        )
    ax.set_xlabel("Initial target range (m)")
    ax.set_ylabel("RMSE (m)")
    ax.set_xticks(np.arange(100, 301, 20))
    set_log_rmse_axis(ax)
    save_figure(fig, output_dir, "rmse_distance_100_300m_detail.png", dpi)
    ...
    robustness_specs = [
        (
            ...
            "r_tdoa_{}",
            MODEL_ORDER,
            r"Sensor-wise static TOA bias level ($\mu$s)",
            10,
        ),
        (
            ...
            "r_doa_{}",
            DOA_MODEL_ORDER,
            r"DOA angular input error std. ($^\circ$)",
            2,
        ),
        (
            ...
            "r_tdoa_std_{}",
            MODEL_ORDER,
            r"Sensor-wise random TOA error std. ($\mu$s)",
            10,
        ),
    ]
...
def plot_trajectory_figures(
    ...
    plane_specs = [
        ((0, 1), "X (m)", "Y (m)", "trajectory_xy_plane.png"),
        ((0, 2), "X (m)", "Z (m)", "trajectory_xz_plane.png"),
        ((1, 2), "Y (m)", "Z (m)", "trajectory_yz_plane.png"),
    ]
    for dims, xlabel, ylabel, filename in plane_specs:
    ...
    save_figure(fig, output_dir, "trajectory_3d.png", dpi)
